		...
		if (this.input_data instanceof JSONArray) {
			...
			for (int index = 0; index < js_arr_obj.length(); index++) {
				System.out.println("Key : " + index + "\nValue : " + js_arr_obj.get(index) + "\nValue Type : "
						+ js_arr_obj.get(index).getClass().getSimpleName());
			...
			for (int index = 0; index < js_obj.length(); index++) {
				System.out.println("Key : " + js_obj.names().getString(index) + "\nValue : "
						+ js_obj.get(js_obj.names().getString(index)) + "\nValue Type : "
						+ js_obj.get(js_obj.names().getString(index)).getClass().getSimpleName());
				...
				case "JSONObject": {
					...
					this.root_composite.add(new Type(js_obj.names().getString(index),
							new CompositeCreator(js_obj.get(js_obj.names().getString(index))).create()));
				}
					break;
				default:
					System.out.println("4) Adding new primitive leaf to " + js_obj.names().getString(index) + "\n");
					this.root_composite.add(
							new Type(js_obj.names().getString(index), js_obj.get(js_obj.names().getString(index))));
					break;
				}
				;
	...
	public void printAllKeys(ArrayList<Type> subtypes_arr) {
		for (int index = 0; index < subtypes_arr.size(); index++) {
			...
			default: {
				...
				// "+subtypes_arr.get(index).getValue().getClass().getSimpleName());
			}
				break;
			}
		}
